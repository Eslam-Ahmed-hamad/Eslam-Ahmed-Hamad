<h1 align="center">Hi there 👋, I'm Eslam Hamad</h1>
<h3 align="center">
Founder & CEO of EH DESIGN ICT • Software Engineer • ERP Systems Developer • AI Automation Engineer
</h3>

<p align="center">
  <a href="https://eslamahmedhamad.com" target="_blank">🌐 Website</a> •
  <a href="mailto:info@ehdesign.online">📧 Email</a> •
  <a href="https://linkedin.com/in/eslamhamd" target="_blank">💼 LinkedIn</a> •
  <a href="https://github.com/eslamhamd" target="_blank">🐙 GitHub</a>
</p>

---

## 🧠 About Me

I'm the founder of **EH DESIGN ICT**, a multi-service tech agency focused on  
**ERP systems, automation, AI agents, and modern web solutions**.

I build end-to-end systems:
- ERP Platforms  
- AI Automation & n8n Workflows  
- Web & API Solutions  
- WordPress & E-Commerce  
- UI/UX & Digital Branding  

From analysis → architecture → development → deployment,  
I deliver real, production-grade systems for businesses.

---

# 🛠 Tech Stack & Tools

## 💻 Languages & Frameworks
![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=c-sharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=flat-square&logo=dotnet&logoColor=white)
![ASP.NET Core](https://img.shields.io/badge/ASP.NET_Core-512BD4?style=flat-square&logo=dotnet&logoColor=white)
![SQL Server](https://img.shields.io/badge/SQL_Server-CC2927?style=flat-square&logo=microsoftsqlserver&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=flat-square&logo=vite&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white)

---

## 🤖 AI & Automation
![OpenAI](https://img.shields.io/badge/OpenAI_Agents-412991?style=flat-square&logo=openai&logoColor=white)
![n8n](https://img.shields.io/badge/n8n_Automation-ff6f00?style=flat-square&logo=n8n&logoColor=white)
![Zapier](https://img.shields.io/badge/Zapier-FF4A00?style=flat-square&logo=zapier&logoColor=white)
![APIs](https://img.shields.io/badge/API_Integration-0066cc?style=flat-square&logo=fastapi&logoColor=white)
![LLMs](https://img.shields.io/badge/LLM_Workflows-7A1FA2?style=flat-square&logo=google&logoColor=white)

---

## 🎨 Design & UI/UX
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=figma&logoColor=white)
![Photoshop](https://img.shields.io/badge/Photoshop-31A8FF?style=flat-square&logo=adobephotoshop&logoColor=white)
![Illustrator](https://img.shields.io/badge/Illustrator-FF9A00?style=flat-square&logo=adobeillustrator&logoColor=white)
![Canva](https://img.shields.io/badge/Canva-00C4CC?style=flat-square&logo=canva&logoColor=white)

---

# 📦 Featured ERP Projects

### 🏋️ EH Gym ERP (Full Gym Management System)
**ASP.NET Core 8 + EF Core + Next.js 15 + SQL Server**  
Multi-module ERP: Members, Subscriptions, Attendance QR, POS, Inventory, Finance, Trainers, Classes, Reports.

🔗 GitHub: https://github.com/eslamhamd/eh-gym-erp

---

### 🛒 Business OS Store – Retail ERP (v1.0.0)
Inventory + POS + Products + Roles  
Security Patch v1.1 Ready.

🔗 GitHub: https://github.com/eslamhamd/business-os-store

---

### 🧱 EH ERP – Web ERP System (v1)
Authentication, Roles, Products, Inventory, Warehouses.

🔗 GitHub: https://github.com/eslamhamd/eh-erp

---

# 🧩 Skill Summary

| Area | Level |
|------|-------|
| ERP Systems (Architecture & Modules) | ⭐⭐⭐⭐⭐ |
| ASP.NET Core / SQL Server | ⭐⭐⭐⭐☆ |
| AI Automation (n8n / LLMs / APIs) | ⭐⭐⭐⭐⭐ |
| Web Development (React / Next.js) | ⭐⭐⭐⭐☆ |
| WordPress & E-Commerce | ⭐⭐⭐⭐☆ |
| Branding, Design & UI/UX | ⭐⭐⭐⭐⭐ |

---

# 🎓 Courses & Learning Path
- System Analysis & Design (DFD, ERD, Use Cases, SDLC)
- ASP.NET Core & SQL Server
- n8n + LLM Agentic Systems
- WordPress & WooCommerce
- Odoo Fundamentals (ERP Concepts)
- Graphic Design & Digital Marketing

---

> “I don’t just write code — I build systems that make businesses run smarter.”

